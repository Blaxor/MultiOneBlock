...
public class IslandName implements SubCommand {
    ...
    public IslandName(OneBlock plugin, String aliases[], String permission, Command parent) {
    ...
    public void execute(CommandSender sender, List<String> args) {
        ...
        if (plugin.getPlayerManager().getPlayer(player.getUniqueId()).getIsland(false) == null) {
            player.sendMessage("Please use /is go first!");
            return;
        }
        Island island = plugin.getPlayerManager().getPlayer(player.getUniqueId()).getIsland(false);
        sender.sendMessage("The name of island is: " + ChatColor.translateAlternateColorCodes('&', island.getMeta().getName()));
    }
