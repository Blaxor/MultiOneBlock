        ...
        if (sender instanceof Player) {
            ...
            if (playerOB.getParticipant().containsKey(SCOPE_CONFIRMATION.INVITE)) {
                ...
                if (playerOB.getParticipant().get(SCOPE_CONFIRMATION.INVITE).get(0) instanceof RequestInvite) {
                    ...
                    Invite.sendResponse(plugin, (Player) sender, new ResponseInvite(invite.invited, invite.inviter, Responses.REJECT));

                }
                sender.sendMessage(plugin.getLangManager().get((Player)sender,MESSAGE.ISLAND_INVITE_REJECTED));
